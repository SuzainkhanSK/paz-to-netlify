...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    WHERE tablename = 'referrals' AND policyname = 'Users can read own referrals as referrer'
  ) THEN
    -- RLS Policies for referrals
    EXECUTE 'CREATE POLICY "Users can read own referrals as referrer"
      ON referrals
      FOR SELECT
      TO authenticated
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'referrals' AND policyname = 'Users can read own referrals as referred'
  ) THEN
    EXECUTE 'CREATE POLICY "Users can read own referrals as referred"
      ON referrals
      FOR SELECT
      TO authenticated
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'referrals' AND policyname = 'Users can insert referrals'
  ) THEN
    EXECUTE 'CREATE POLICY "Users can insert referrals"
      ON referrals
      FOR INSERT
      TO authenticated
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'referral_earnings' AND policyname = 'Users can read own referral earnings'
  ) THEN
    EXECUTE 'CREATE POLICY "Users can read own referral earnings"
...
-- Function to process transaction commission
