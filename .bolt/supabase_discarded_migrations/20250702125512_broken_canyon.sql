/*
  ...
  1. New Tables
    ...
    - `referral_leaderboard` (materialized view)
      ...
      - `level_1_referrals` (integer)
      - `level_2_referrals` (integer)
      - `level_3_referrals` (integer)
  ...
  3. Functions
    ...
    - Calculate and award referral commissions
    - Refresh leaderboard
*/
...
SELECT 
  ...
  p.email,
  p.full_name,
  COUNT(DISTINCT CASE WHEN r.level = 1 THEN r.referred_id END) as level_1_referrals,
  COUNT(DISTINCT CASE WHEN r.level = 2 THEN r.referred_id END) as level_2_referrals,
  COUNT(DISTINCT CASE WHEN r.level = 3 THEN r.referred_id END) as level_3_referrals,
  COUNT(DISTINCT r.referred_id) as total_referrals,
  COALESCE(SUM(CASE WHEN r.status = 'completed' THEN r.points_awarded ELSE 0 END), 0) as bonus_earnings,
  COALESCE(SUM(re.commission_points), 0) as commission_earnings,
  COALESCE(SUM(CASE WHEN r.status = 'completed' THEN r.points_awarded ELSE 0 END), 0) + COALESCE(SUM(re.commission_points), 0) as total_earnings,
  now() as last_updated
FROM profiles p
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_referral_leaderboard_user_id 
ON referral_leaderboard(user_id);
...
-- Create RLS policy for referral_leaderboard
CREATE POLICY "Users can view referral leaderboard"
