  ...
  const checkCompletedTasks = async () => {
    ...
    try {
      ...
      if (isSupabaseConfigured) {
        ...
        if (!error && data) {
          // Extract original task IDs by removing the date suffix
          const dbCompletedTaskIds = data.map(item => {
            const taskId = item.task_id;
            // Remove date suffix if present (format: taskX-YYYY-MM-DD)
  ...
  const handleClaimReward = async (taskId: string) => {
    ...
    try {
      ...
      if (isSupabaseConfigured && user?.id) {
        // Create unique task ID by appending today's date
        const today = new Date().toISOString().split('T')[0];
        const uniqueTaskId = `${taskId}-${today}`;
